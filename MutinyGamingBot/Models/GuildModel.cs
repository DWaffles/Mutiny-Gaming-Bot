...
namespace MutinyBot.Models
{
    ...
    public class GuildModel
    {
        ...
        /// <summary>
        /// Gets the Id of the mute role set within the guild, if any.
        /// </summary>
        ...
        /// <param name="discordMember">A list DiscordMember objects that belong to this guild.</param>
        public void UpdateGuild(DiscordGuild discordGuild, IEnumerable<DiscordMember> discordMember)
        {
            ...
            if (MuteRoleId != 0 && !discordGuild.Roles.ContainsKey(MuteRoleId))
                MuteRoleId = 0;
            ...
            var newMembers = discordMember.ToDictionary(x => x.Id, x => x);
            foreach (var eMember in Members)
            {
                var dMember = discordMember.SingleOrDefault(m => m.Id == eMember.MemberId);
